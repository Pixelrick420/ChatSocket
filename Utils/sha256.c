...
static void sha256Block(sha256_t *sha) {
...
static void sha256AppendByte(sha256_t *sha, uint8_t byte) {
...
char *createHashedPass(const char *roomName, const char *password) {
  ...
  size_t buf_len =
      strlen(salt_hex) + 1 + strlen(roomName) + 1 + strlen(inner_hex) + 1;
  ...
  for (int i = 1; i < HASH_ITERATIONS; i++) {
    ...
    memcpy(tmp, out_hash, SHA256_HEX_SIZE);
    sha256Hex(tmp, SHA256_HEX_SIZE - 1, out_hash);
  }
